...
# 🚀 ROTA ATUALIZADA: GERAR PIX REAL NO MERCADO PAGO (COM CORREÇÃO DE FUSO E TELEFONE) 🚀
@app.post("/gerar-pix-reserva/")
def criar_reserva(booking: BookingRequest, db: Session = Depends(get_db)):
    # 1. CORREÇÃO DO FUSO HORÁRIO: Ajustando o relógio de Londres para o Brasil (UTC-3)
    agora_brasil = datetime.utcnow() - timedelta(hours=3)
    
    # Damos uma tolerância de 15 minutos caso o cliente demore preenchendo
    ...
    # SALVA A RESERVA PRIMEIRO (Para gerar o ID no banco)
    ...
    db.add(nova_reserva)
    db.commit()
    db.refresh(nova_reserva) # Pega o ID que acabou de ser criado
    
    # 2. CORREÇÃO DO TELEFONE: Remove parênteses e traços para o Mercado Pago não travar
    ...
    try:
        ...
        if "point_of_interaction" not in payment:
            # Se der erro no MP, apaga a reserva fantasma
            db.delete(nova_reserva)
            db.commit()
            raise Exception("O Mercado Pago recusou a geração do Pix.")
        ...
        db.delete(nova_reserva)
        db.commit()
        raise HTTPException(status_code=400, detail=f"Erro no Mercado Pago: {str(e)}")
