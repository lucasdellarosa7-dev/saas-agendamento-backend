...
def criar_reserva(booking: BookingRequest, db: Session = Depends(get_db)):
    ...
    # 1. SALVA A RESERVA PRIMEIRO (Para gerar o ID no banco)
    ...
    # 2. CHAMA O MERCADO PAGO COM O ID DA RESERVA
    email_falso = f"cliente_{booking.telefone_cliente.replace(' ', '')}@teste.com" if booking.telefone_cliente else "cliente_anonimo@teste.com"

    payment_data = {
        "transaction_amount": float(deposit_amount),
        "description": f"Sinal Reserva - {servico.nome_servico}",
        "payment_method_id": "pix",
        "external_reference": str(nova_reserva.id), # O TRUQUE MÁGICO AQUI
        "payer": {
            "email": email_falso,
    ...
    try:
        ...
        if "point_of_interaction" not in payment:
            # Se der erro no MP, apaga a reserva que criamos
    ...
    # 3. ATUALIZA A RESERVA COM O CÓDIGO PIX REAL
