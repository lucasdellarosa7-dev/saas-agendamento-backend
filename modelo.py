...
def fazer_login(tenant_id: int, req: LoginRequest, db: Session = Depends(get_db)):
    if req.tipo_acesso == "dono":
        ...
        if not negocio or negocio.senha_dono != req.senha:
        ...
        if not prof or prof.senha != req.senha:
            raise HTTPException(status_code=401, detail="Senha incorreta")
...
def listar_profissionais(tenant_id: int, db: Session = Depends(get_db)):
    ...
    resultado = [{"id": p.id, "nome": p.nome} for p in profs]
...
def criar_servico(tenant_id: int, servico: ServiceCreate, db: Session = Depends(get_db)):
    # NOVO: Trava de segurança no Python (0 a 50%)
    if servico.max_deposito_pct < 0 or servico.max_deposito_pct > 50:
        raise HTTPException(status_code=400, detail="O sinal deve ser entre 0 e 50%.")

    novo = ServiceDB(
        tenant_id=tenant_id, nome_servico=servico.nome_servico,
        tipo_preco=servico.tipo_preco, preco_base=servico.preco_base,
        max_deposito_pct=servico.max_deposito_pct,
    ...
    for prof_id in servico.profissionais_ids:
        link = ServiceProfessionalLink(service_id=novo.id, professional_id=prof_id)
        db.add(link)
    db.commit()
...
def listar_servicos(tenant_id: int, db: Session = Depends(get_db)):
    ...
    for s in servicos:
        ...
        profissionais_info = [{"id": p.id, "nome": p.nome} for p in profs]
        nomes_profs = [p.nome for p in profs]
        ...
        resultado.append({
            ...
            "max_deposito_pct": s.max_deposito_pct,
            "profissionais_nomes": nomes_profs,
            "profissionais_ids": prof_ids,
            "profissionais_info": profissionais_info
        })
...
def editar_servico(tenant_id: int, servico_id: int, servico: ServiceCreate, db: Session = Depends(get_db)):
    # NOVO: Trava de segurança na edição também
    if servico.max_deposito_pct < 0 or servico.max_deposito_pct > 50:
        raise HTTPException(status_code=400, detail="O sinal deve ser entre 0 e 50%.")

    db_servico = db.query(ServiceDB).filter(ServiceDB.id == servico_id, ServiceDB.tenant_id == tenant_id).first()
    if db_servico:
        db_servico.nome_servico = servico.nome_servico
        db_servico.preco_base = servico.preco_base
        db_servico.duracao_minutos = servico.duracao_minutos
        db_servico.max_deposito_pct = servico.max_deposito_pct
        
        db.query(ServiceProfessionalLink).filter(ServiceProfessionalLink.service_id == servico_id).delete()
        for prof_id in servico.profissionais_ids:
            link = ServiceProfessionalLink(service_id=servico_id, professional_id=prof_id)
            db.add(link)
...
def buscar_horarios(tenant_id: int, data: str, service_id: int, professional_id: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    if professional_id:
        if professional_id not in prof_ids_habilitados:
            return {"data": data, "horarios_disponiveis": []}
        prof_ids_habilitados = [professional_id]
    
    profissionais = db.query(ProfessionalDB).filter(
        ProfessionalDB.tenant_id == tenant_id,
        ProfessionalDB.id.in_(prof_ids_habilitados)
    ).all()
    
    if not negocio or not servico or not profissionais:
        return {"data": data, "horarios_disponiveis": []}
    ...
    for prof in profissionais:
        reservas_prof = db.query(BookingDB).filter(
            BookingDB.tenant_id == tenant_id,
            BookingDB.professional_id == prof.id,
            BookingDB.data_hora_inicio >= data_inicio,
            BookingDB.data_hora_inicio <= data_fim
        ).order_by(BookingDB.data_hora_inicio.asc()).all()
        
        blocos_livres = []
        tempo_atual = horario_abertura
        
        for reserva in reservas_prof:
            if tempo_atual < reserva.data_hora_inicio:
                blocos_livres.append((tempo_atual, reserva.data_hora_inicio))
            tempo_atual = max(tempo_atual, reserva.data_hora_fim)
            
        if tempo_atual < horario_fechamento:
            blocos_livres.append((tempo_atual, horario_fechamento))
        ...
        for inicio_bloco, fim_bloco in blocos_livres:
            ...
            while slot_atual + timedelta(minutes=servico.duracao_minutos) <= fim_bloco:
                if slot_atual > agora:
                    horarios_livres_set.add(slot_atual.strftime("%H:%M"))
                slot_atual += timedelta(minutes=servico.duracao_minutos)
                
    horarios_livres = sorted(list(horarios_livres_set))
    return {"data": data, "horarios_disponiveis": horarios_livres}
...
def criar_reserva(booking: BookingRequest, db: Session = Depends(get_db)):
    ...
    links = db.query(ServiceProfessionalLink).filter(ServiceProfessionalLink.service_id == booking.service_id).all()
    prof_ids_habilitados = [link.professional_id for link in links]
    
    if booking.professional_id:
        if booking.professional_id not in prof_ids_habilitados:
            raise HTTPException(status_code=400, detail="Profissional não faz este serviço.")
        prof_ids_habilitados = [booking.professional_id]
    
    profissionais = db.query(ProfessionalDB).filter(
        ProfessionalDB.tenant_id == servico.tenant_id,
        ProfessionalDB.id.in_(prof_ids_habilitados)
    ).all()
    ...
    for prof in profissionais:
        conflito = db.query(BookingDB).filter(
            BookingDB.tenant_id == servico.tenant_id,
            BookingDB.professional_id == prof.id,
            BookingDB.data_hora_fim > booking.data_hora,
            BookingDB.data_hora_inicio < hora_fim
        ).first()
        
        if not conflito:
            prof_escolhido = prof.id
            break
            
    if not prof_escolhido:
        raise HTTPException(status_code=400, detail="Nenhum profissional livre neste horário.")
    ...
    pix_code = f"PIX-{deposit_amount}-{servico.nome_servico.replace(' ', '')}"
    
    nova_reserva = BookingDB(
        tenant_id=servico.tenant_id, service_id=servico.id, professional_id=prof_escolhido,
        nome_cliente=booking.client_name, data_hora_inicio=booking.data_hora, data_hora_fim=hora_fim,
        status_pagamento="PENDENTE", valor_total=final_price, valor_sinal=deposit_amount, codigo_pix=pix_code
    )
    db.add(nova_reserva)
    db.commit()
    return {"mensagem": "Reserva iniciada!", "codigo_pix": pix_code}
...
def ver_agenda(tenant_id: int, data: Optional[str] = None, professional_id: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    if professional_id:
        query = query.filter(BookingDB.professional_id == professional_id)
    ...
    for r in reservas:
        ...
        nome_prof = prof.nome if prof else "Qualquer um"
        
        servico = db.query(ServiceDB).filter(ServiceDB.id == r.service_id).first()
        nome_servico = servico.nome_servico if servico else "Serviço deletado"
